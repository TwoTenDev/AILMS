...
async def _upload_scorm(zip_path, module_id, moodle_url, token, kb_path):
    ...
    try:
        ...
        # Add SCORM activity via docker exec into ailms-moodle-1
        zip_filename = os.path.basename(zip_path)

        def _docker_install():
            import docker, tarfile, io as _io
            client = docker.from_env()
            moodle = client.containers.get("ailms-moodle-1")

            # Copy PHP installer from chatbot container into Moodle container
            buf = _io.BytesIO()
            with tarfile.open(fileobj=buf, mode="w") as tar:
                tar.add("/app/add_scorm.php", arcname="add_scorm.php")
            buf.seek(0)
            moodle.put_archive("/tmp/", buf)

            # Copy SCORM zip into Moodle container
            buf2 = _io.BytesIO()
            with tarfile.open(fileobj=buf2, mode="w") as tar:
                tar.add(zip_path, arcname=zip_filename)
            buf2.seek(0)
            moodle.put_archive("/tmp/", buf2)

            # Run PHP installer
            code, out = moodle.exec_run([
                "php", "/tmp/add_scorm.php",
                f"--courseid={course_id}",
                f"--zippath=/tmp/{zip_filename}",
                f"--name={course_title}",
            ])
            return code, (out or b"").decode("utf-8", errors="replace").strip()

        code, out = await asyncio.get_event_loop().run_in_executor(None, _docker_install)
        if code != 0 or not out.startswith("OK:"):
            return {"ok": True, "course_id": course_id, "course_title": course_title,
                    "warn": f"SCORM activity not added: {out}"}
